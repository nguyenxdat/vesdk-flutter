        ...
        self.present(mediaEditViewControllerBlock: { (configurationData, serializationData) -> MediaEditViewController? in
            ...
            IMGLY.bundleImageBlock = { imageName in
              ...
              case "imgly_icon_delete_48pt":
                let image = UIImage(systemName: "trash.fill", withConfiguration: config)?.withTintColor(UIColor.red, renderingMode: .alwaysOriginal)
                return image
              default:
                return nil
              }
            ...
            // (red: 255/255, green: 74/255, blue: 114/255, alpha: 1)

            VESDK.localizationBlock = { stringToLocalize in
                  switch stringToLocalize {
                  case "pesdk_editor_title_exportingVideoSpinner":
                    return "Saving video"
                  default:
                    return nil
                  }
